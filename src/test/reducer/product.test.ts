...
describe("Testing actions from product reducer", () => {
    ...
    test("Should delete a product from backend server", async()=>{
        await store.dispatch(fetchAllProducts())
        console.log(store.getState().productReducer.length)
        await store.dispatch(deleteAproduct(testProductData[0]))
